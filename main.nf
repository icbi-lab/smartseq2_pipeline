...
raw_reads = params.reads
out_dir = file(params.outputDir)
pattern = params.pattern
mode = params.publishDirMode
star_index = file(params.references.RefIdx)
rsem_ref = file(params.references.RsemRef)
anno_file = file(params.references.Annotation)
spec = params.species
nameref = params.references.refName
read_pair_fq = Channel.fromFilePairs("${raw_reads}/${pattern}", type: 'file')
read_pair_st = Channel.fromFilePairs("${raw_reads}/${pattern}", type: 'file')
read_pair_tcr = Channel.fromFilePairs("${raw_reads}/${pattern}", type: 'file')
read_pair_bcr = Channel.fromFilePairs("${raw_reads}/${pattern}", type: 'file')
    ...
    if (!params.rsem_ref && params.fasta) {
        process make_rsem_reference {
            publishDir path: { params.save_reference ? "${params.outdir}/reference_genome" : params.outdir },
                        saveAs: { params.save_reference ? it : null }, mode: '$mode'
            input:
                file "genome.fa" from file(params.fasta)
                file "annotation.gtf" from file(params.gtf)
            ...
            script:
            """
            rsem-prepare-reference --gtf annotation.gtf genome.fa rsem
            """
        }
    } else {
        rsem_ref = Channel
 ...
 */
process fastqc {
    publishDir "$out_dir/Quality_Control/${sample}_fastqc", mode: "$mode"
    input:
        set sample, file(in_fastq) from read_pair_fq
 ...
 */
process STAR {
    publishDir "$out_dir/Expression/${sample_fq}_STAR", mode: "$mode"
    input:
        set sample_fq, file(in_fastq) from read_pair_st
        file star_index
    output:
    ...
    STAR --runThreadN ${task.cpus} --genomeDir ${star_index} \$TMP \
            --readFilesIn ${raw_reads}/${in_fastq.get(0)} ${raw_reads}/${in_fastq.get(1)} \
...
process featureCounts {
    publishDir "$out_dir/Expression", mode: "$mode"
    input:
 ...
 */
process multiqc {
    publishDir "$out_dir/Quality_Control", mode: "$mode"

    input:
    file ('fastqc/*') from fastqc_files.collect().ifEmpty([])
    file ('star/*') from bam_mqc.collect()
    file ('featureCounts/*') from count_mqc.collect()
    file ('rsem/*') from rsem_mqc.collect()
...
process summarize_FC {
    ...
    output:
        file("*_resfc.txt") into result_files
...
process summmarize_TPM {
    ...
    output:
        file("*_restpm.txt") into result_files1
...
process make_result_files {
    ...
    input:
        file x from result_files.collect()
        file y from count_files2.collect()
        file z from result_files1.collect()
 ...
 */
process TraCeR{
    publishDir "$out_dir/TraCeR", mode: "$mode"

    input:
        set sample, file(in_tracer) from read_pair_tcr
    ...
    script:
    """
    tracer assemble -p ${task.cpus} -s ${spec} \
 ...
 */
process BraCeR{
    publishDir "$out_dir/BraCeR", mode: "$mode"

    input:
        set sample, file(in_bracer) from read_pair_bcr
    ...
    script:
    """
    bracer assemble -p ${task.cpus} -s ${spec} \
